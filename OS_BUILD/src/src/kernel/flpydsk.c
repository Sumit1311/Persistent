...
bool flpydsk_initialize_dma(uint8_t* buffer, unsigned length) {
...
void flpydsk_control_motor(bool b) {
	...
	if (b)
		flpydsk_write_dor(
				_CurrentDrive | motor | FLPYDSK_DOR_MASK_RESET
						| FLPYDSK_DOR_MASK_DMA);
	else
...
//! configure drive
void flpydsk_drive_data(uint32_t stepr, uint32_t loadt, uint32_t unloadt,
		bool dma) {
