...
void
flpydsk_send_command(uint8_t cmd)
{
...
void
flpydsk_check_int(uint32_t* st0, uint32_t* cyl)
{
...
flpydsk_control_motor(bool b)
{
  ...
  if (b)
    flpydsk_write_dor(
        _CurrentDrive | motor | FLPYDSK_DOR_MASK_RESET | FLPYDSK_DOR_MASK_DMA);
  else
...
//! configure drive
void
flpydsk_drive_data(uint32_t stepr, uint32_t loadt, uint32_t unloadt, bool dma)
{
...
int
flpydsk_calibrate(uint32_t drive)
{
