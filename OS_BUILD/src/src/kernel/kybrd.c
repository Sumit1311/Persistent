...
enum KYBRD_CTRL_IO
{
  KYBRD_CTRL_STATS_REG = 0x64, KYBRD_CTRL_CMD_REG = 0x64
};
...
enum KYBRD_ERROR
{
  ...
  KYBRD_ERR_BUF_OVERRUN = 0, KYBRD_ERR_ID_RET = 0x83AB, KYBRD_ERR_BAT = 0xAA,	//note: can also be L. shift key make code
...
void
kybrd_ctrl_send_cmd(uint8_t);
uint8_t
kybrd_enc_read_buf();
void
kybrd_enc_send_cmd(uint8_t);
void
i86_kybrd_irq();
...
void
kybrd_ctrl_send_cmd(uint8_t cmd)
{
...
void
kybrd_enc_send_cmd(uint8_t cmd)
{
...
kkybrd_get_last_key()
{
  ...
  return
      (_scancode != INVALID_SCANCODE) ?
          ((KEYCODE) _kkybrd_scancode_std[_scancode]) : (KEY_UNKNOWN);
}
...
char
kkybrd_key_to_ascii(KEYCODE code)
{
